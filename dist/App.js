...
function clearForm() {
    ...
    if (DOM.dateInput)
        DOM.dateInput.value = "";
    if (DOM.pendingRadio)
        DOM.pendingRadio.checked = false;
    if (DOM.completedRadio)
...
function clearSubtaskForm() {
    if (DOM.subtaskTitleInput)
        DOM.subtaskTitleInput.value = "";
    if (DOM.subtaskDescInput)
        DOM.subtaskDescInput.value = "";
    if (DOM.subtaskPriorityInput)
        DOM.subtaskPriorityInput.value = "";
    if (DOM.subtaskPendingRadio)
        DOM.subtaskPendingRadio.checked = false;
    if (DOM.subtaskCompletedRadio)
...
export function openSubtaskModal(taskId) {
    if (!DOM.subtaskModal)
        return;
    if (DOM.subtaskParentIdInput)
        DOM.subtaskParentIdInput.value = taskId.toString();
    clearSubtaskForm();
...
if (DOM.closeSubtaskBtn)
    DOM.closeSubtaskBtn.addEventListener("click", closeSubtaskModal);
if (DOM.clearSubtaskBtn)
    DOM.clearSubtaskBtn.addEventListener("click", clearSubtaskForm);
if (DOM.addSubtaskBtn) {
    DOM.addSubtaskBtn.addEventListener("click", () => {
        ...
        if (!selectedStatus) {
            if (DOM.subtaskStatusError)
                DOM.subtaskStatusError.classList.remove("d-none");
            isValid = false;
        }
        else {
            if (DOM.subtaskStatusError)
...
if (tasks.length === 0) {
    ...
    if (DOM.loadingMessage)
        DOM.loadingMessage.classList.remove("d-none");
    fetchTasksFromAPI()
        .then((apiTasks) => {
        setTasks(apiTasks);
        saveTasks();
        renderTasks();
        if (DOM.loadingMessage)
            DOM.loadingMessage.classList.add("d-none");
        if (DOM.errorMessage)
            DOM.errorMessage.classList.add("d-none");
    })
        .catch((error) => {
